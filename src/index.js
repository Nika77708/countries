...
const refs = {
  ...
  country_list: document.querySelector('ul'),
  country_info: document.querySelector('.country-info'),
};

function createCard(el) {
  const card = document.createElement('div');
  card.classList.add("div_list");
...
//   console.log(Object.values(languages).join(", "));
  ...
  img.classList.add("img_one_card");
  ...
  p.innerText = population;

  const lang = document.createElement('p');
  lang.innerText = Object.values(languages).join(", ");

  card.append(img, h2, h3, p, lang);

  refs.country_info.append(card);

  refs.input.value = "";
  
}

function renderCountries(resp) {
  const arr = resp.map(el => createCard(el));
  refs.country_list.append(...arr);
}

function searchCountries() {
  const value = refs.input.value;
  refs.country_list.replaceChildren();
  refs.country_info.replaceChildren();
...
// Поле поиска
// Название страны для поиска пользователь вводит в текстовое поле input#search-box. HTTP-запросы выполняются при наборе имени страны,
// то есть по событию input. Но, делать запрос при каждом нажатии клавиши нельзя, так как одновременно получится много запросов и они
// будут выполняться в непредсказуемом порядке.

// Необходимо применить приём Debounce на обработчике события и делать HTTP-запрос спустя 300мс после того, как пользователь перестал
// вводить текст. Используй пакет lodash.debounce - https://www.npmjs.com/package/lodash.debounce .

// Если пользователь полностью очищает поле поиска, то HTTP-запрос не выполняется, а разметка списка стран или информации о стране
// пропадает.

// Выполни санитизацию введенной строки методом trim(), это решит проблему когда в поле ввода только пробелы или они есть в начале и
// в конце строки.
...
// Для уведомлений используй библиотеку notiflix (https://github.com/notiflix/Notiflix#readme) и выводи такую строку:
// "Too many matches found. Please enter a more specific name.".

// Если бэкенд вернул от 2-х до 10-х стран, под тестовым полем отображается список найденных стран. Каждый элемент списка состоит из
// флага и имени страны.
// Если результат запроса это массив с одной страной, в интерфейсе отображается разметка карточки с данными о стране: флаг, название,
// столица, население и языки.
...
// Добавь уведомление "Oops, there is no country with that name" в случае ошибки используя библиотеку
// notiflix (https://github.com/notiflix/Notiflix#readme).

// ВНИМАНИЕ
// Не забывай о том, что fetch не считает 404 ошибкой, поэтому необходимо явно отклонить промис чтобы можно было словить
//и обработать ошибку.
